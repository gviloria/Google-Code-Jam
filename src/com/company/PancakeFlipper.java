...
/**
 * Problem
 * Last year, the Infinite House of Pancakes introduced a new kind of pancake. It has a happy face made of
 ...
 * pancake flipper that flips exactly K consecutive pancakes. That is, in that range of K pancakes, it
 * changes every happy-side pancake to a blank-side pancake, and vice versa; it does not change the left-to-right
 * order of those pancakes.
 *
 * You cannot flip fewer than K pancakes at a time with the flipper, even at the ends of the row
 * (since there are raised borders on both sides of the cooking surface). For example, you can flip the
 * first K pancakes, but not the first K - 1 pancakes.
 *
 * Your apprentice cook, who is still learning the job, just used the old-fashioned single-pancake flipper
 * to flip some individual pancakes and then ran to the restroom with it, right before the time when customers
 * come to visit the kitchen. You only have the oversized pancake flipper left, and you need to use it quickly
 * to leave all the cooking pancakes happy side up, so that the customers leave feeling happy with their visit.
 *
 ...
 * Each consists of one line with a string S and an integer K. S represents the row of pancakes:
 ...
 * (starting from 1) and y is either IMPOSSIBLE if there is no way to get all the pancakes happy side up,
 * or an integer representing the the minimum number of times you will need to use the oversized pancake flipper
 * to do it.
 *
 * Limits
 * 1 ≤ T ≤ 100.
 ...
 * getting to ++++-++-, then the rightmost 3, getting to ++++---+, and finally the 3 pancakes that remain
 * blank side up. There are other ways to do it with 3 flips or more, but none with fewer than 3 flips.
 *
